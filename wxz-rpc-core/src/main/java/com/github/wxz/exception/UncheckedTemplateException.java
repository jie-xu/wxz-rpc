...
import com.github.wxz.http.ui.template.HttpTemplate;
...
public class UncheckedTemplateException extends RuntimeException {
    ...
    public static UncheckedTemplateException invalidNumberOfArguments(int argsNum) {
        String msg = HttpTemplate.template(INVALID_NUMBER_OF_ARGUMENTS).arg("argsNum", argsNum).fmt();
        return new UncheckedTemplateException(msg);
    }

    public static UncheckedTemplateException argumentAlreadyExist(String arg) {
        String msg = HttpTemplate.template(ARGUMENT_ALREADY_DEFINED).arg("arg", arg).fmt();
        return new UncheckedTemplateException(msg);
    }

    public static UncheckedTemplateException invalidCharacterInParam(char c, int idx) {
        String msg = HttpTemplate.template(INVALID_CHARACTER_IN_PARAM_NAME).args("char", c, "idx", idx).fmt();
        return new UncheckedTemplateException(msg);
    }

    public static UncheckedTemplateException ioExceptionReadingFromFile(String strPath, Throwable t) {
        String msg = HttpTemplate.template(IO_EXCEPTION_READING_FROM_FILE).arg("strPath", strPath).fmt();
        return new UncheckedTemplateException(msg, t);
    }

    public static UncheckedTemplateException invalidArgumentName(Object argName) {
        String msg = HttpTemplate.template(INVALID_ARGUMENT_NAME_NULL_OR_EMPTY, "arg", argName).fmt();
        return new UncheckedTemplateException(msg);
    }

    public static UncheckedTemplateException invalidStateException(HttpTemplate.State state) {
        String msg = HttpTemplate.template(INVALID_STATE_EXCEPTION, "state", state).fmt();
        return new UncheckedTemplateException(msg);
    }
